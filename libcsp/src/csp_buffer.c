...
static unsigned int count, size_buffer;
...
int csp_buffer_init(int buf_count, int buf_size) {
	...
	count = buf_count;
	size_buffer = buf_size + CSP_BUFFER_PACKET_OVERHEAD;
	unsigned int skbfsize = (sizeof(csp_skbf_t) + size_buffer);
...
void *csp_buffer_get_isr(size_t buf_size) {
	...
	if (buf_size + CSP_BUFFER_PACKET_OVERHEAD > size_buffer)
		return NULL;
...
void *csp_buffer_clone(void *buffer) {
	...
	if (clone)
		memcpy(clone, packet, size_buffer);
